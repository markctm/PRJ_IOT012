...
TaskHandle_t TaskWifi_monitor_hdlr;
                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_num < EXAMPLE_ESP_MAXIMUM_RETRY) {
           ...
           // vTaskDelete(TaskWifi_monitor_hdlr);
            NO_SIGNAL();
...
void TaskWifi_monitor(void )
{
    ...
    for(;;)
    {
            ...
            if((ap.rssi - rssi_old_value > 10) || (ap.rssi - rssi_old_value < -10))
            {
                ...
                if((ap.rssi<=-50)&&(ap.rssi>-60)){
                    SIGNAL_STRENGHT_4();
                }
                if((ap.rssi<=-60)&&(ap.rssi>-70)){
                    SIGNAL_STRENGHT_3();
                }
                if((ap.rssi<=-70)&&(ap.rssi>-80)){
                    SIGNAL_STRENGHT_2();
                }
                if(ap.rssi<=-80){
                    SIGNAL_STRENGHT_1();
